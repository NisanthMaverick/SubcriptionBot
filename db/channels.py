...
class ChannelQueries(ConnectionManager):
    ...
    def get_premium_channel(self, channel_id: int) -> Optional[Dict[str, Any]]:
        for url in self._db_urls:
            ...
            try:
                with self._get_cursor(specific_url=url) as (cursor, conn):
                    cursor.execute("SELECT channel_id, title, invite_link FROM premium_channels WHERE channel_id = %s", (channel_id,))
                    row = cursor.fetchone()
                    if row:
    ...
    def get_all_premium_channels(self) -> List[Dict[str, Any]]:
        for url in self._db_urls:
            ...
            try:
                with self._get_cursor(specific_url=url) as (cursor, conn):
                    cursor.execute("SELECT channel_id, title, invite_link FROM premium_channels ORDER BY title ASC")
    ...
    def check_user_access_to_channel(self, user_id: int, channel_id: int) -> bool:
        ...
        # If the channel is not mapped to any plan, we allow access by default (or treat as unmonitored).
        is_mapped = False
        for url in self._db_urls:
            if self._db_status.get(url) != "Online":
                continue
            try:
                with self._get_cursor(specific_url=url) as (cursor, conn):
                    cursor.execute("SELECT COUNT(*) FROM channel_mappings WHERE channel_id = %s", (channel_id,))
    ...
    def get_all_channel_mappings(self) -> list:
        for url in self._db_urls:
            ...
            try:
                with self._get_cursor(specific_url=url) as (cursor, conn):
                    cursor.execute("SELECT channel_id, plan_id FROM channel_mappings")
                    rows = cursor.fetchall()
